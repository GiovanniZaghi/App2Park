...
class EditOptionsCam extends StatefulWidget {
  @override
  _EditOptionsCamState createState() => _EditOptionsCamState();
}

class _EditOptionsCamState extends State<EditOptionsCam>
  ...
  String ticket_date_time_now =
  DateFormat("yyyy-MM-dd HH:mm:ss").format(DateTime.now()).toString();
  String dataSave =
  DateFormat("yyyy-MM-dd_HH:mm:ss").format(DateTime.now()).toString();
  ...
  Future<void> requestPermission() async {
    if (await Permission.camera.request().isUndetermined ||
        await Permission.microphone.request().isUndetermined ||
        await Permission.storage.request().isUndetermined) {
  ...
  Future<String> takePicture() async {
    ...
    TicketHistoricPhotoOffModel(
        0, 0, id_historic_status, filePath, ticket_date_time_now);

    id_ticket_historic_photo = await ticketHistoricPhotoDao
    ...
    if (connectivityResult == ConnectivityResult.mobile ||
      ...
      if (properties.width > properties.height) {
        _image = await FlutterNativeImage.compressImage(filePath,
            quality: 80,
            targetWidth: 600,
      ...
      var res1 =
      await sendForm(urlRequest + 'api/tickets/newtickethistoricphoto', {
        'id_park': '${park.id}',
        'id_historic_photo_app': '$id_ticket_historic_photo',
        'id_ticket_historic_app': '$id_historic_status',
        'id_ticket_historic': '$id_historic_on',
        'date_time': '$ticket_date_time_now'
      }, {
        'file': _image
      });
      if (res1.statusCode == 200) {
        ...
        TicketHistoricPhotoResponse ticketHistoricPhotoRes =
        TicketHistoricPhotoResponse.fromJson(res1.data);

        TicketHistoricPhotoModel ticketHistoricPhotoModel =
            ticketHistoricPhotoRes.data;

        print(ticketHistoricPhotoModel);

        bool ok = await ticketHistoricPhotoDao.updateTicketHistoricPhotoIdOn(
            int.tryParse(ticketHistoricPhotoModel.id),
            int.tryParse(ticketHistoricPhotoModel.id_ticket_historic),
            ticketHistoricPhotoModel.photo,
            id_ticket_historic_photo);
