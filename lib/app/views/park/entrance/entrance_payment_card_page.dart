...
class EntrancePaymentCard extends StatefulWidget {
  @override
  _EntrancePaymentCardState createState() => _EntrancePaymentCardState();
}

class _EntrancePaymentCardState extends State<EntrancePaymentCard> {
    ...
    Widget exitButton = FlatButton(
      ...
      onPressed: () async{
        if (_formKey.currentState.validate()) {
          ...
          int id_cash_movement = await cashMovementDao.saveCashMovement(cashMovementOff);
          ticketsDao.updateTicketsPayUntil(sharedPref.read("horario_saida"), id_ticket_app);
          ...
          if (connectivityResult == ConnectivityResult.mobile ||
            ...
            Ticket ticket = Ticket();
            ticket.pay_until = await sharedPref.read("horario_saida");
            TicketResponse ticketRes = await TicketService.updateTicketOn(ticket, id_ticket);
