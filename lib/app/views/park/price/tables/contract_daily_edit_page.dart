...
class ContractDailyEdit extends StatefulWidget {
  @override
  _ContractDailyEditState createState() => _ContractDailyEditState();
}

class _ContractDailyEditState extends State<ContractDailyEdit> {
  ...
  loadSharedPrefs() async {
    try {
      ...
      AgreementsOff a =
      AgreementsOff.fromJson(await sharedPref.read("daily"));
      ...
      LogOff logOff = LogOff('0', null, null, e.toString(), 'IN PROD VERSION', now.toString(), 'ERRO CONTRACT DAILY EDIT PAGE', 'APP');
                        ...
                        if (FormInput.get("Login").validate()) {
                          ...
                          if(platesList.length == 0){
                            ...
                            if (!validador) {
                              ...
                              DateTime now = DateTime.now();
                              LogOff logOff = LogOff('0', id_user.toString(), id_park.toString(), erros.toString(), vers, now.toString(), 'CONTRATOS - ERRO AO EDITAR CONTRATO MENSALISTA', 'APP');
                              LogDao logDao = LogDao();
                              logDao.saveLog(logOff);
                              alertModalsError(
